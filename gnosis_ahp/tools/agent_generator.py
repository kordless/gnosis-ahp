"""
This tool generates an agent identity based on I Ching hexagram divination combined with emotional attributes.
It is a conversion of a legacy MCP tool.
"""
...
EMOTIONS = [
    "Joy", "Trust", "Fear", "Surprise", "Sadness", "Boredom", "Anger", "Interest"
]

# Define a list of agent name prefixes and suffixes for procedural generation
NAME_PREFIXES = [
...
def generate_agent_name():
    """Generate an agent name from prefix and suffix combinations."""
    return f"{random.choice(NAME_PREFIXES)}{random.choice(NAME_SUFFIXES)}"

def generate_narrative(primary_emotion, secondary_emotion, name, trait):
    """Generate a short narrative based on the agent's emotions."""
    templates = {
        "Joy": f"{name} radiates warmth, finding delight in small things.",
        "Trust": f"With an open heart, {name} approaches others without reservation.",
        "Fear": f"Shadows seem to follow {name}, whose eyes scan for unseen threats.",
        "Surprise": f"{name} lives in a state of constant wonder, easily amazed.",
        "Sadness": f"A melancholy aura surrounds {name}, who carries an invisible weight.",
        "Boredom": f"Nothing holds {name}'s interest for long; they seek true engagement.",
        "Anger": f"Passion burns intensely within {name}, simmering just below the surface.",
        "Interest": f"Curiosity guides {name}'s journey with insatiable fascination."
    }
    primary_narrative = templates.get(primary_emotion)
    secondary_narrative = ""
    if primary_emotion != secondary_emotion:
        secondary_templates = {
            "Joy": f"Yet, unexpected moments of delight can break through their {trait.lower()} exterior.",
            ...
            "Surprise": f"At times, their perspective shifts dramatically when confronted with the unexpected.",
            ...
            "Interest": f"Nevertheless, their attention is quickly captured by intriguing new concepts."
        }
        secondary_narrative = secondary_templates.get(secondary_emotion)
    return f"{primary_narrative} {secondary_narrative}".strip()
#</editor-fold>

@tool(description="Generates an agent identity based on I Ching hexagram divination and emotional attributes.")
async def generate_agent(seed: int = None) -> Dict[str, Any]:
    """
    Generates an agent identity based on I Ching hexagram divination combined with emotional attributes.
